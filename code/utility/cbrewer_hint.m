function [colormap]=cbrewer_hint(ctype, cname, ncol, interp_method)
%
...
arguments
    % - ctype: type of color table 'seq' (sequential), 'div' (diverging), 'qual' (qualitative)
    ctype char {mustBeMember(ctype, {'seq', 'div', 'qual'})}
    % - cname: name of colortable. It changes depending on ctype.
    cname char {mustBeMember(cname, {'Blues','BuGn','BuPu','GnBu','Greens', ...
        'Greys','Oranges','OrRd','PuBu','PuBuGn','PuRd','Purples','RdPu', ...
        'Reds', 'YlGn', 'YlGnBu', 'YlOrBr', 'YlOrRd', 'Spectral', 'BrBG', ...
        'PiYG', 'PRGn', 'PuOr', 'RdBu', 'RdGy', 'RdYlBu', 'RdYlGn', ...
        'Accent', 'Dark2', 'Paired', 'Pastel1', 'Pastel2', 'Set1', 'Set2', 'Set3'})}
    ncol double = 12
    interp_method char {mustBeMember(interp_method, {'cubic', 'linear', 'nearest', 'spline'})} = 'linear'
end
...
if (~exist('interp_method', 'var'))
    ...
    % Cubic convolution
    % interp_method='cubic'; % default
    % Shape-preserving piecewise cubic interpolation
    interp_method='pchip'; % By Ziqi
end
...
colormap(colormap >= 0.999) = 0.999; % By Ziqi
end

function z = mustBeColor(z, type)
switch type
    case 'seq'
        %disp('Sequential tables:')
        z={'Blues','BuGn','BuPu','GnBu','Greens','Greys','Oranges','OrRd','PuBu','PuBuGn','PuRd',...
